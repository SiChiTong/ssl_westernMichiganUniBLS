...
// Public
//---------------------------
void ToolMapGenerator::SetSample(double constant_attenuation, double constant_pathLossExponent, double x, double y, double z)
{
	if(MapType_=="2D") z=0;

	constant_attenuation_.push_back(constant_attenuation);
	constant_pathLossExponent_.push_back(constant_pathLossExponent);
	xs_.push_back(x);
	ys_.push_back(y);
	zs_.push_back(z);
...
double ToolMapGenerator::GetConstant_Attenuation(double x, double y, double z)
{
	if(MapType_=="2D") z=0;
	...
	for(int i=0;i<xs_.size();i++)
	{
		...
		double weightedConstant = constant_attenuation_[i]*weight;
...
double ToolMapGenerator::GetConstant_PathLossExponent(double x, double y, double z)
{
	if(MapType_=="2D") z=0;
	...
	for(int i=0;i<xs_.size();i++)
	{
		...
		double weightedConstant = constant_pathLossExponent_[i]*weight;
...
void ToolMapGenerator::OutputSamples(string filename)
{
	...
	for(int i=0;i<xs_.size();i++)
	{
		write<<xs_[i]<<" "<<ys_[i]<<" "<<zs_[i]<<" "<<constant_attenuation_[i]<<" "<<constant_pathLossExponent_[i]<<endl;
	}
...
bool ToolMapGenerator::ImportSamples(string filename)
{
	...
	ys_.clear();
	zs_.clear();
	constant_attenuation_.clear();
	constant_pathLossExponent_.clear();

	// read
	while(!read.eof())
	{
		double x, y, z, catt, cple;
		read>>x>>y>>z>>catt>>cple;
		...
		ys_.push_back(y);
		zs_.push_back(z);
		constant_attenuation_.push_back(catt);
		constant_pathLossExponent_.push_back(cple);
	...
	for(int i=0;i<xs_.size();i++)
	{
		cout<<"Sample ID "<<i<<": "<<xs_[i]<<", "<<ys_[i]<<", "<<zs_[i]<<"; "<<constant_attenuation_[i]<<endl;
	}
	*/
...
void ToolMapGenerator::GetMap(int binx, double minx, double maxx,int biny, double miny, double maxy,int binz, double minz, double maxz)
{
	...
	for(int i=0;i<binx;i++)
	{
		...
		double constant = GetConstant_Attenuation(x,y,z);
...
void ToolMapGenerator::GetMap_PathLossExponent(int binx, double minx, double maxx,int biny, double miny, double maxy,int binz, double minz, double maxz)
{
	if(MapType_=="2D") {
		binz = 1;
		minz = 0;
	...
	ofstream write("log_mapOfPathLossExponent.txt");
	write<<binx<<" "<<minx<<" "<<maxx<<" "<<biny<<" "<<miny<<" "<<maxy<<" "<<binz<<" "<<minz<<" "<<maxz<<endl;
	...
	for(int i=0;i<binx;i++)
	{
		...
		double constant = GetConstant_PathLossExponent(x,y,z);
